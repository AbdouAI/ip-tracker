...
const ipShown=document.getElementById("ipAdrs")
...
function ff(urlPar){
    ...
    .then(()=>{
        if(info.ip){
            for(ctry of countryList){
                if(ctry.code==info.location.country){
                    var fullCountry=ctry.name
                }
            }
            ipShown.textContent=info.ip
            adrs.textContent=`${fullCountry},${info.location.city} ${info.location.postalCode}`
            timeZone.textContent="GMT"+info.location.timezone
...
ff(`https://geo.ipify.org/api/v2/country,city?apiKey=at_nMcV2DJm1cDrkPzdPCzE0HyLd931g`)
